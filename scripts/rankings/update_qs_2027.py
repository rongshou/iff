#!/usr/bin/env python3
"""Update qs_rank in advisor.db to QS World University Rankings 2027"""

import os
import sqlite3

# QS World University Rankings 2027 (complete dataset from topuniversities.com / qs.com)
QS_2027 = {
    ...
    "University of Science & Technology of China": 81,
    "Duke University": 82,
    "University of Glasgow": 82,
    "University of Copenhagen": 84,
...
LIVE_DB = '/home/admin/tianquan/backend/data/advisor.db'
BACKUP_DB = '/home/admin/tianquan/backup/advisor/data/advisor.db'

for db_path in [LIVE_DB, BACKUP_DB]:
    ...
    for cn_name, en_name in cn_en.items():
        if en_name not in QS_2027:
            continue
        new_rank = QS_2027[en_name]
        
        row = cur.execute('SELECT id, qs_rank FROM universities WHERE name = ?', (cn_name,)).fetchone()
        if not row:
            not_found.append((cn_name, en_name))
            continue
        
        db_id, old_rank = row
        if old_rank == new_rank:
            already_correct.append((cn_name, en_name, new_rank))
            continue
        
        cur.execute('UPDATE universities SET qs_rank = ? WHERE id = ?', (new_rank, db_id))
        updated.append((cn_name, en_name, old_rank, new_rank))

    cur.execute('COMMIT')

    # Summary
    print(f"更新: {len(updated)} | 已正确: {len(already_correct)} | 未找到: {len(not_found)}")
    if updated:
        print(f"\n{'中文名':<20} {'英文名':<35} {'旧QS':>6} → {'新QS':>6}")
        print("-" * 75)
        for cn_name, en_name, old, new in sorted(updated, key=lambda x: x[3]):
            arrow = "↑" if old and new and old > new else "↓"
            print(f"{cn_name:<20} {en_name:<35} {old or '?':>6} → {new:>6} {arrow}")

    conn.close()

print("\nDone! 同时更新了 live DB 和 backup DB。")
